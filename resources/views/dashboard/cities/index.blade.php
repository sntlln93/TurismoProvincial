            ...
            <div class="container-modal">
                ...
                <div >
                    <form action="{{ url('panel-de-administracion/cities') }}" method="POST" class="modal-body">
                        @csrf
                        <div><h4>Nombre:</h4><input class="@error('name') error-input @enderror" type="text" name="name" value="" placeholder="Nombre de la localidad"></div>
                        @error('name') <small class="error-message">{{ $message }}</small> @enderror
                        
                        <div
                            ><h4>Descripción:</h4>
                            <textarea class="@error('description') error-input @enderror msj" name="description" maxlength="1000" rows="5" placeholder="Escribí una descripción de la localidad"></textarea>
                        </div>
    ...
    <div class="articles">
        @foreach($district->cities as $city)
            <section class="article d-flex">
                ...
                <div class="icon">
                    <a href="{{ url('panel-de-administracion/cities/'.$city->id.'/edit') }}" class="btn-edit"><i class="icon-edit"></i></a>
                </div>
            </section>
        @endforeach
    </div> 
