    ...
    <main>
        ...
        <div class="modal-body view">
            <form action="{{ url('panel-de-administracion/cities/'.$city->id) }}" method="POST">
                @csrf @method('PUT')
                <div><h4>Nombre:</h4><input class="@error('name') error-input @enderror" type="text" name="name" value="{{ $city->name }}" placeholder="Nombre de la localidad"></div>
                @error('name') <small class="error-message">{{ $message }}</small> @enderror

                <div>
                    <h4>Descripción:</h4>
                    <textarea class="@error('description') error-input @enderror" name="description" class="msjEdit" maxlength="1000" rows="6" placeholder="Escribí una descripción">{{ $city->description }}</textarea>
                </div>
