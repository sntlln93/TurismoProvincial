                        ...
                        @foreach($subtype->services as $service)
                            <div class="article">
                                <div class="article-info">
                                    ...
                                    <div class="info-2">
                                        <b>{{ $type->name == "Alojamiento" ? "Check in:" : "Abre a las:"}}</b> {{ $service->start->format('h:i') }} hs </br>
                                        <b>{{ $type->name == "Alojamiento" ? "Check out:" : "Cierra a las:"}}</b> {{ $service->end->format('h:i') }} hs </br>
                                        <b>Subtipo:</b> {{ $subtype->name }}
                                    </div>
                                </div>
                                <div class="icon">
                                    <a href="{{ url('panel-de-administracion/services/'.$service->id) }}" class="btn-show"><i class="icon-info-circled"></i></a>
