...
<main>
    ...
    <form action="{{ url('dashboard/activities/'.$activity->id) }}" method="POST" class="modal-body">
        @csrf @method('PUT')
        <div>
            <h4>Nombre:</h4><input class="@error('name') error-input @enderror" type="text" name="name"
                value="{{ $activity->name }}" placeholder="">
        </div>
        ...
        <div>
            <h4>Responsable:</h4><input class="@error('responsable') error-input @enderror" type="text"
                name="responsable" value="{{ $activity->responsable }}" placeholder="">
        </div>
        @error('responsable') <small class="error-message">{{ $message }}</small> @enderror

        <div>
            <h4>Precio:</h4><input class="@error('amount') error-input @enderror" type="number" name="amount"
                value="{{ $activity->amount }}" placeholder="">
        </div>
        ...
        <div>
            ...
            <select class="@error('location_id') error-input @enderror" name="location_id" id="">
                ...
                <option value="{{ $location->id }}" @if($location->id == $activity->location_id) selected
        ...
        <div>
            <h4>Inicio:</h4><input class="@error('start') error-input @enderror" type="datetime-local" id="start"
                name="start" value="{{ $activity->start->format('Y-m-d') }}T{{ $activity->start->format('h:i') }}"
                placeholder="">
        </div>
        @error('start') <small class="error-message">{{ $message }}</small> @enderror

        <div>
            <h4>Fin:</h4><input class="@error('end') error-input @enderror" type="datetime-local" id="end"
                name="end" value="{{ $activity->end->format('Y-m-d') }}T{{ $activity->end->format('h:i') }}"
                placeholder="">
        </div>
        ...
        <div>
            ...
            <textarea class="@error('description') error-input @enderror" name="description" rows="6"
                placeholder="Escribe aquí la descripción">{{ $activity->description }}</textarea>
        </div>
